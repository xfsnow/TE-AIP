...
```
The `AzureWebJobsStorage` value is not required for this exercise, so you can leave it blank.The `AzureWebJobsStorage` value is not required for this exercise, so you can leave it blank.

To retrieve the required AZURE_TENANT_ID value:
1. Run the following command in a terminal window to get the tenant ID for your Azure subscription:

   ```bash
   az account show --query tenantId -o tsv
   ```
2. Copy the tenant ID value and paste it into the `AZURE_TENANT_ID` value of the `local.settings.json` file.
...
4. In the **Select Function App** command pallet dialog that appears at the top of Visual Studio Code:
   ...
   **Note:** If you are using a Microsoft internal subscription, your subscription may not appear in the list of subscriptions. If this is the case, press **Ctrl+Shift+P** to open the command palette and input `Azure: Sign in to Tenant`. Sign in and choose your tenant. Follow the guidance to sign in to your Azure account. After signing in, repeat the steps to deploy the function app.
